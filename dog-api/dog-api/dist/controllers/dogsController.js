 ...
 */
export const getBreedDog = async (req, res) => {
    try {
 ...
 */
export const saveFavoriteDog = async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO public.favorite_dogs (name, breed, image_url) VALUES ($1, $2, $3) RETURNING *', [name, breed, image_url]);
 ...
 */
export const getFavoriteDogs = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM public.favorite_dogs ORDER BY create_at DESC');
        res.json({
            success: true,
            data: {
